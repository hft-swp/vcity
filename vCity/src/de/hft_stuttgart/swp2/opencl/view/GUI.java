...
public class GUI extends JFrame implements GLEventListener, KeyListener,
	...
	private GUI() throws OpenClException {
		...
		// Vertex d0 = ShadowCalculatorInterface.vertexDiff(t.getVertices()[1],
		// t.getVertices()[0]);
		// Vertex d1 = ShadowCalculatorInterface.vertexDiff(t.getVertices()[2],
		// t.getVertices()[0]);
		...
		for (int j = 1; j < 13; ++j) {
			for (int i = 0; i < sunPositions[j - 1].length; i++) {
				...
				sunPositions[j - 1][i] = new SunPositionCalculator(
						utcCal.getTime(), parser);
			}
		}
		...
		calc.calculateShadow(ShadowPrecision.VERY_LOW, splitAzimuth,
				splitHeight); // VERY_LOW(5f), LOW(2.5f), MID(1.25f),
								// HIGH(0.75f), ULTRA(0.375f), HYPER(0.1f),
								// AWESOME(0.01f)
		...
		System.out
				.printf("\n\n"
						...
						milliParser, milliShadowCalculation,
						milliseconds2string(milliShadowCalculation),
						(double) milliShadowCalculation
								/ City.getInstance().getBuildings().size(),
						City.getInstance().getBuildings().size(),
						milliTotalTime, milliseconds2string(milliTotalTime));
				...
				for (int j = 0; j < surface.getPolygons().size(); ++j) {
					...
					for (Triangle t : p.getTriangles()) {
						...
						for (int sunPositionIdx = 0; sunPositionIdx < sunPositions[month].length; sunPositionIdx++) {
							gl.glVertex3d(
									sunPositions[month][sunPositionIdx].getX(),
									sunPositions[month][sunPositionIdx].getY(),
									sunPositions[month][sunPositionIdx].getZ());
						}
						gl.glEnd();
